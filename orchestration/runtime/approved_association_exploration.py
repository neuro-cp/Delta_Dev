...
def load_explorations(runtime_root: str | Path) -> tuple[AssociationExploration, ...]:
    ...
    return tuple(AssociationExploration(
        ...
        approval_request_id=str(item.get("approval_request_id") or ""), inquiry_question=str(item.get("inquiry_question") or ""),
...
def build_inquiry_question(graph: ProvisionalSemanticGraphState, source_ids: tuple[str, ...], edge_ids: tuple[str, ...]) -> str:
    ...
    return (
        "Return JSON only with shared_structure, possible_implication, relation_limits, uncertainty, and suggested_next_question. "
        "Explore this explicit dependency without presenting it as fact or validation. "
...
def queue_exploration(
    ...
    record = AssociationExploration(
        ...
        approval_request_id=approval_request_id, inquiry_question=build_inquiry_question(graph, source_record_ids, relation_edge_ids),
...
def execute_once(
    ...
    *, executor: Callable[[str, Mapping[str, Any]], Mapping[str, Any]] | None = None,
    ...
    if request["lifecycle_state"] == "approved":
        request = ledger.execute_claimed_request(request["request_id"], {"executor": executor} if executor else None)
...
def _parse_insight(raw: str, exploration: AssociationExploration) -> dict[str, str] | None:
    ...
    if any(term in combined for term in ("validated", "certain", "operator approved")):
        return None
