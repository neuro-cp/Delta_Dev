...
| Metric | Value |
| --- | ---: |
| runtime_grade | `PASS WITH ISSUES` |
| read_only_verified | `True` |
| grounding_score | `1.0` |
| hallucinations | `0.0` |
| retrieval_precision | `0.16` |
| retrieval_recall | `0.5333` |
| attention_precision | `0.55` |
| attention_recall | `0.4333` |
| noise_used_in_reasoning | `12.0` |
...
| top_recurring_lexical_attractors | `[['resource', 23], ['evidence', 21], ['emergency', 20], ['failure', 20], ['uncertainty', 14], ['plan', 13], ['risk', 10], ['response', 5], ['change', 2], ['capacity', 2]]` |
| top_recurring_noisy_concepts | `[['e0038487-2363-4ba8-8e2e-a1b465c44ee0', 8], ['5252e50d-b97e-4d57-a79a-b9d64ddbbec7', 8], ['5ac48263-cbff-4380-bf63-fc9500bcad8d', 8], ['0586e881-099d-4866-a1cf-4ed571139c70', 8], ['cb4f7649-cbdd-4cb3-905d-6f5f69832e9c', 7], ['06b4d255-58d5-49f5-adec-43426ce0d09b', 7], ['65e4ca55-7169-4ee4-b388-a6927043a92d', 6], ['17a8d30a-9f2d-4d15-8100-d4bf18ca2e71', 6], ['89b85467-0655-47d3-9a0c-89da364cd487', 6], ['b85b0a01-9c3f-4c42-a26a-01741c84682c', 6]]` |

## Activation Ranking Candidates

### Prototype 0: generic-token dampening plus specificity bonus simulation

Problem: Relevant concepts are often buried below broad concepts that match generic attractor tokens rather than distinctive query terms.

Evidence:
- mean expected rank = 10
- expected concepts outside top 10 = 8
- mean noise above expected = 9.7083
- top lexical attractors = resource, evidence, emergency, failure, uncertainty, plan, risk, response
- activation-ranking primary cases = 3
...
### Query coverage floor for specific activation
...
Exact metrics expected to move:
- retrieval_precision increases
- sparse activation decreases
...
### Duplicate/cluster diversity in activation window
...
Exact metrics expected to move:
- top-10 concept diversity increases
- expected_outside_top_10 decreases
...
## Attention Scoring Candidates

### Bounded attention rescue pass

Problem: Expected concepts in top 10 are sometimes pruned before working memory.

Evidence:
- attention-primary cases = 3
...
## Sparse Abstention Candidates

### Sparse activation abstention gate

Problem: Unsupported or sparse prompts still activate unrelated candidate knowledge.

Evidence:
- sparse abstention-needed cases = 2
...
Runtime V1.3 prototype 0: read-only scoring simulation for generic-token dampening plus specificity bonus.

## Why this prototype should come before the others

The ranking diagnostic shows recurring generic lexical attractors and 8 expected concepts outside the top-10 activation window. Simulating score changes first can test rank movement without changing live runtime behavior.
