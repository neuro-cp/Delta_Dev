...
MODULE_NAME = "orchestration.runtime.arc_21_multi_runtime_collaboration"
...
def test_arc_21_object_construction():
    m = module()
    objects = m.build_primitives()
    assert len(objects) == 8
    assert set(m.PRIMITIVE_NAMES) == {'RuntimeRegistry', 'RuntimeIdentity', 'SharedEvidenceProtocol', 'SharedKnowledgeProtocol', 'CollaborationPlanner', 'RuntimeNegotiation', 'ConsensusProtocol', 'CollaborationAudit'}
    assert all(obj.as_dict()["review_state"] == "review_required" for obj in objects)


def test_arc_21_validation_and_safety_flags():
    m = module()
...
def test_arc_21_demo_and_report_payload_json_serializable():
    m = module()
...
def test_arc_21_write_report_outputs_review_only_artifacts():
    m = module()
