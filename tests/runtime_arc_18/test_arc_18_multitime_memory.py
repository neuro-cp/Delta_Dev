...
MODULE_NAME = "orchestration.runtime.arc_18_multitime_memory"
...
def test_arc_18_object_construction():
    m = module()
    objects = m.build_primitives()
    assert len(objects) == 10
    assert set(m.PRIMITIVE_NAMES) == {'ImmediateMemory', 'WorkingMemory', 'EpisodicMemory', 'SemanticMemory', 'ProceduralMemory', 'LongTermMemory', 'ArchiveMemory', 'MemoryPromotionRules', 'MemoryDecaySimulation', 'MemoryAudit'}
    assert all(obj.as_dict()["review_state"] == "review_required" for obj in objects)


def test_arc_18_validation_and_safety_flags():
    m = module()
...
def test_arc_18_demo_and_report_payload_json_serializable():
    m = module()
...
def test_arc_18_write_report_outputs_review_only_artifacts():
    m = module()
