...
MODULE_NAME = "orchestration.runtime.arc_11_controlled_integration"
...
def test_arc_11_object_construction():
    m = module()
    objects = m.build_primitives()
    assert len(objects) == 12
    assert set(m.PRIMITIVE_NAMES) == {'IntegrationPlanner', 'IntegrationValidator', 'KnowledgeDiff', 'IntegrationPreview', 'IntegrationApprovalPipeline', 'IntegrationCommitTransaction', 'RollbackExecutorPlan', 'VersionGraphUpdatePlan', 'IntegrationAudit', 'KnowledgeHealthRecalculation', 'IntegrationSimulationReplay', 'RegressionComparison'}
    assert all(obj.as_dict()["review_state"] == "review_required" for obj in objects)


def test_arc_11_validation_and_safety_flags():
    m = module()
...
def test_arc_11_demo_and_report_payload_json_serializable():
    m = module()
...
def test_arc_11_write_report_outputs_review_only_artifacts():
    m = module()
