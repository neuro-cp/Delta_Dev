...
MODULE_NAME = "orchestration.runtime.arc_17_world_model"
...
def test_arc_17_object_construction():
    m = module()
    objects = m.build_primitives()
    assert len(objects) == 11
    assert set(m.PRIMITIVE_NAMES) == {'WorldEntity', 'WorldEvent', 'WorldLocation', 'WorldOrganization', 'WorldTimeline', 'WorldState', 'WorldRelationship', 'WorldCausalityGraph', 'WorldConsistencyValidator', 'WorldVersioning', 'WorldAudit'}
    assert all(obj.as_dict()["review_state"] == "review_required" for obj in objects)


def test_arc_17_validation_and_safety_flags():
    m = module()
...
def test_arc_17_demo_and_report_payload_json_serializable():
    m = module()
...
def test_arc_17_write_report_outputs_review_only_artifacts():
    m = module()
