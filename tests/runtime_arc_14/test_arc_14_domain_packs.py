...
MODULE_NAME = "orchestration.runtime.arc_14_domain_packs"
...
def test_arc_14_object_construction():
    m = module()
    objects = m.build_primitives()
    assert len(objects) == 12
    assert set(m.PRIMITIVE_NAMES) == {'DomainPackRegistry', 'DomainActivationProfile', 'ProgrammingPack', 'FinancePack', 'SciencePack', 'MedicalPack', 'LegalPack', 'EngineeringPack', 'OperationsPack', 'CrossDomainCoordinator', 'DomainConflictAnalyzer', 'DomainCoverageMetrics'}
    assert all(obj.as_dict()["review_state"] == "review_required" for obj in objects)


def test_arc_14_validation_and_safety_flags():
    m = module()
...
def test_arc_14_demo_and_report_payload_json_serializable():
    m = module()
...
def test_arc_14_write_report_outputs_review_only_artifacts():
    m = module()
