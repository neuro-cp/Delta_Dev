...
MODULE_NAME = "orchestration.runtime.arc_13_sleep_replay"
...
def test_arc_13_object_construction():
    m = module()
    objects = m.build_primitives()
    assert len(objects) == 13
    assert set(m.PRIMITIVE_NAMES) == {'ReplaySchedulerSimulation', 'ReplayBatchBuilder', 'ReplayPriorityEngine', 'ConsolidationPlanner', 'DuplicateMergePlanner', 'AbstractionProposal', 'ForgettingProposal', 'KnowledgeCompressionSimulation', 'LongTermMemoryPlan', 'SleepCycleState', 'ReplayMetrics', 'ConsolidationAudit', 'ReplayTimeline'}
    assert all(obj.as_dict()["review_state"] == "review_required" for obj in objects)


def test_arc_13_validation_and_safety_flags():
    m = module()
...
def test_arc_13_demo_and_report_payload_json_serializable():
    m = module()
...
def test_arc_13_write_report_outputs_review_only_artifacts():
    m = module()
