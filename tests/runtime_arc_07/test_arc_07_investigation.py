...
MODULE_NAME = "orchestration.runtime.arc_07_investigation"
...
def test_arc_07_object_construction():
    m = module()
    objects = m.build_primitives()
    assert len(objects) == 12
    assert set(m.PRIMITIVE_NAMES) == {'Investigation', 'ProblemDefinition', 'KnowledgeGapAnalysis', 'ResearchQuestion', 'InvestigationPlan', 'EvidenceCollectionPlan', 'Finding', 'InvestigationGraph', 'InvestigationRecommendation', 'MultiInvestigationState', 'InvestigationReflection', 'InvestigationTransaction'}
    assert all(obj.as_dict()["review_state"] == "review_required" for obj in objects)


def test_arc_07_validation_and_safety_flags():
    m = module()
...
def test_arc_07_demo_and_report_payload_json_serializable():
    m = module()
...
def test_arc_07_write_report_outputs_review_only_artifacts():
    m = module()
