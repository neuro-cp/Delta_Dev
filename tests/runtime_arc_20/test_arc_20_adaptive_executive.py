...
MODULE_NAME = "orchestration.runtime.arc_20_adaptive_executive"
...
def test_arc_20_object_construction():
    m = module()
    objects = m.build_primitives()
    assert len(objects) == 11
    assert set(m.PRIMITIVE_NAMES) == {'GoalPrioritizer', 'ContextSwitchPlanner', 'InterruptManager', 'ResourceAllocator', 'ExecutivePolicyEngine', 'ObjectiveOptimizer', 'RiskBalancer', 'ExecutiveReflectionLoop', 'ExecutiveSimulation', 'ExecutiveTimeline', 'ExecutiveAudit'}
    assert all(obj.as_dict()["review_state"] == "review_required" for obj in objects)


def test_arc_20_validation_and_safety_flags():
    m = module()
...
def test_arc_20_demo_and_report_payload_json_serializable():
    m = module()
...
def test_arc_20_write_report_outputs_review_only_artifacts():
    m = module()
