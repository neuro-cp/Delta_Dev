...
MODULE_NAME = "orchestration.runtime.arc_16_cognitive_os"
...
def test_arc_16_object_construction():
    m = module()
    objects = m.build_primitives()
    assert len(objects) == 12
    assert set(m.PRIMITIVE_NAMES) == {'CognitiveProcess', 'CognitiveThread', 'CognitiveContext', 'RuntimeSchedulerSimulation', 'CognitiveInterrupt', 'ContextSwitchManager', 'CognitiveLifecycle', 'RuntimeIsolation', 'KernelProcessManager', 'ContextPersistenceModel', 'RuntimeHealthMonitor', 'ProcessAudit'}
    assert all(obj.as_dict()["review_state"] == "review_required" for obj in objects)


def test_arc_16_validation_and_safety_flags():
    m = module()
...
def test_arc_16_demo_and_report_payload_json_serializable():
    m = module()
...
def test_arc_16_write_report_outputs_review_only_artifacts():
    m = module()
