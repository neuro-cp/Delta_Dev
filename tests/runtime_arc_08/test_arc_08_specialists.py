...
MODULE_NAME = "orchestration.runtime.arc_08_specialists"
...
def test_arc_08_object_construction():
    m = module()
    objects = m.build_primitives()
    assert len(objects) == 13
    assert set(m.PRIMITIVE_NAMES) == {'Specialist', 'SpecialistRegistry', 'SpecialistCapabilityProfile', 'SpecialistSelectionPlan', 'SpecialistReasoningOutput', 'ParallelDeliberation', 'ConsensusSummary', 'SpecialistConflictBundle', 'MetaReasoningCritique', 'ConfidenceFusion', 'DeliberationGraph', 'SpecialistTransaction', 'SpecialistPerformanceMetrics'}
    assert all(obj.as_dict()["review_state"] == "review_required" for obj in objects)


def test_arc_08_validation_and_safety_flags():
    m = module()
...
def test_arc_08_demo_and_report_payload_json_serializable():
    m = module()
...
def test_arc_08_write_report_outputs_review_only_artifacts():
    m = module()
