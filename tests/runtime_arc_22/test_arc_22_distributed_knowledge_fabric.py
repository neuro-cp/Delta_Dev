...
MODULE_NAME = "orchestration.runtime.arc_22_distributed_knowledge_fabric"
...
def test_arc_22_object_construction():
    m = module()
    objects = m.build_primitives()
    assert len(objects) == 9
    assert set(m.PRIMITIVE_NAMES) == {'FabricRegistry', 'KnowledgeReplica', 'SyncPlanner', 'MergePlanner', 'ConflictResolver', 'TrustPropagation', 'ProvenanceReplication', 'FabricHealth', 'FabricAudit'}
    assert all(obj.as_dict()["review_state"] == "review_required" for obj in objects)


def test_arc_22_validation_and_safety_flags():
    m = module()
...
def test_arc_22_demo_and_report_payload_json_serializable():
    m = module()
...
def test_arc_22_write_report_outputs_review_only_artifacts():
    m = module()
