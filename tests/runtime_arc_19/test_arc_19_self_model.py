...
MODULE_NAME = "orchestration.runtime.arc_19_self_model"
...
def test_arc_19_object_construction():
    m = module()
    objects = m.build_primitives()
    assert len(objects) == 10
    assert set(m.PRIMITIVE_NAMES) == {'SelfModel', 'CapabilityInventory', 'LimitationRegistry', 'ConfidenceProfile', 'RuntimeHealthState', 'ActiveGoalRegistry', 'PendingReviewRegistry', 'RuntimeLoadModel', 'SelfExplanationEngine', 'SelfAudit'}
    assert all(obj.as_dict()["review_state"] == "review_required" for obj in objects)


def test_arc_19_validation_and_safety_flags():
    m = module()
...
def test_arc_19_demo_and_report_payload_json_serializable():
    m = module()
...
def test_arc_19_write_report_outputs_review_only_artifacts():
    m = module()
