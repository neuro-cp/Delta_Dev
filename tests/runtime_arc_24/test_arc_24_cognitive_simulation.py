...
MODULE_NAME = "orchestration.runtime.arc_24_cognitive_simulation"
...
def test_arc_24_object_construction():
    m = module()
    objects = m.build_primitives()
    assert len(objects) == 8
    assert set(m.PRIMITIVE_NAMES) == {'SimulationContext', 'SimulationScenario', 'CounterfactualWorld', 'OutcomePredictor', 'BranchExplorer', 'ScenarioComparison', 'SimulationConfidence', 'SimulationAudit'}
    assert all(obj.as_dict()["review_state"] == "review_required" for obj in objects)


def test_arc_24_validation_and_safety_flags():
    m = module()
...
def test_arc_24_demo_and_report_payload_json_serializable():
    m = module()
...
def test_arc_24_write_report_outputs_review_only_artifacts():
    m = module()
